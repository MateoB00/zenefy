...
export default function BlocPrestations({ serviceData }) {
  ...
  React.useEffect(() => {
    const fetchService = async () => {
      const getServicesPartnerCompany = await getServicesByPartner(
        serviceData["partner_company"].id
      );
  ...
  const handleDateTimeChange = async (e) => {
    ...
    if (serviceChoice.average_time === "10 min")
      endedAt.setMinutes(endedAt.getMinutes() + 10);
    if (serviceChoice.average_time === "30 min")
      endedAt.setMinutes(endedAt.getMinutes() + 30);
    if (serviceChoice.average_time === "1 h")
      endedAt.setMinutes(endedAt.getHours() + 1);
    if (serviceChoice.average_time === "2 h")
      endedAt.setMinutes(endedAt.getHours() + 2);

    const reservationsInfos = {
      started_at: startedAt,
      ended_at: endedAt,
      service: serviceChoice.id,
      partner_company: serviceData["partner_company"],
      priceOfService: serviceChoice.price,
    ...
    <div>
      <div className="bloc_prestations">
        {servicesPartnerCompany ? (
          Object.values(servicesPartnerCompany).map(
